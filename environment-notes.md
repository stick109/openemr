...
- 2026-05-01: Symptom: production Railway database operations cannot proceed when `railway whoami` and `railway status` return `Unauthorized. Please run railway login again.` Likely cause: the local Railway OAuth session expired or the refresh token is invalid. Workaround: run `railway login` from this checkout, confirm `railway status` succeeds for the intended project/environment/service, then rerun the database operation. Follow-up: before any Railway production database copy, verify the target service and take a production dump backup first.
