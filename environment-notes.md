...
- 2026-04-30: Symptom: `docker compose ... exec ... php -r ...` one-liners from PowerShell can lose or mangle nested double quotes, producing PHP errors such as `syntax error, unexpected end of file` or `Undefined constant "LOG_LEVEL"` when checking environment variables. Likely cause: the command crosses PowerShell, the Windows native command-line parser for `docker.exe`, Docker Compose, and PHP CLI; PowerShell does not use backslash to escape quotes, and embedded quotes that look valid in the transcript may not survive as intended. Workaround: avoid nested quoted `php -r` probes through Docker when a simpler command exists; use `docker compose -p openemr exec -T openemr printenv LOG_LEVEL`, `php -i` plus host-side `Select-String`, or feed multi-line PHP on stdin instead of packing it into `-r`. For native commands that need literal double quotes, do not use `\"`; use PowerShell-appropriate quoting and verify with a read-only probe first. Follow-up: prefer quote-light diagnostics for Docker/container checks.
