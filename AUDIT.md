...
OpenEMR is a strong foundation for a Clinical Co-Pilot because it already contains the core primitives a trustworthy clinical agent needs: authenticated sessions, role and ACL checks, OAuth2/SMART/FHIR APIs, patient-scoped document storage, event hooks, audit logging, and a large amount of structured clinical data. The audit also shows that these primitives are not yet enough to safely attach an AI agent that can answer patient-specific clinical questions under time pressure. The central risk is not whether the model can retrieve data. It is whether every retrieved fact is authorized, current, attributable, and handled under HIPAA-grade controls.

The highest-impact security finding is patient-level access control. API requests pass through authorization strategies and route-level ACL checks, but the current codebase still depends heavily on broad ACL categories such as `patients\demo`, `patients\med`, and `encounters\auth_a`. The FHIR patient-bound flow has a `checkUserHasAccessToPatient()` hook in `src\RestControllers\Authorization\BearerTokenAuthorizationStrategy.php`, but it currently returns `true`. That is acceptable only if an earlier launch or token context has already proven patient binding. It is not sufficient for an agent tool that might be asked about any patient in natural language. The agent must enforce an explicit "requesting user can access this patient" check before any retrieval, preferably based on appointment, provider, facility, care-team, or current chart context.

The highest-impact privacy finding is logging. OpenEMR's audit posture is mature for a legacy EHR, but the default `api_log_option` is full API logging, and `ApiResponseLoggerListener` can duplicate JSON responses into both `request_body` and `response`. For normal APIs this is already sensitive. For an AI agent, prompts, intermediate tool outputs, source snippets, and generated summaries could all contain PHI. Agent observability must therefore be separate from raw PHI logging: log request IDs, user IDs, patient IDs, source record IDs, timings, token counts, model names, and verification outcomes, while redacting or hashing clinical text unless there is a deliberate retention policy.

Performance is the next gating concern. The target workflow is a physician with roughly 90 seconds between rooms, so the agent must return useful context in seconds. OpenEMR's service layer can search patients, encounters, medications, documents, appointments, allergies, procedures, and FHIR resources, but many paths go through legacy bootstrapping, global state, SQL audit logging, broad table scans, or unbounded result sets if callers do not provide tight filters. The agent should not synchronously crawl the whole chart. It needs a bounded retrieval plan: today's appointment, recent encounters, active problems, active medications, recent labs, allergies, and documents already parsed or explicitly selected.

The architecture is transitional: direct PHP pages, legacy controllers, REST/FHIR controllers, Symfony events, ADODB helpers, emerging Doctrine DBAL/ORM, Twig, Smarty, modules, and site-scoped storage all coexist. The agent should integrate through existing authenticated UI/API boundaries rather than reading the database directly from a standalone service. A custom module or namespaced service under `src` can work, but it must use existing session, ACL, audit, CSRF, and event mechanisms.

Data quality will shape answer quality. Clinical data is spread across `patient_data`, `lists`, `lists_medication`, `prescriptions`, `form_encounter`, form tables, documents, procedure tables, and FHIR projections. Many important fields allow empty strings, null dates, free text, stale records, or duplicate representations. Therefore the agent's first production-grade feature should be an evidence layer that normalizes source records, timestamps them, tags their provenance, and refuses unsupported claims.
